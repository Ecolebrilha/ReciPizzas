...
const finalizarCompra = () => {
    seleciona('.cart--finalizar').addEventListener('click', () => {
        ...
        const numeroWhatsApp = "5581985127220";

        let mensagemPedido = "Olá! Gostaria de fazer o seguinte pedido:\n";

        cart.forEach((item, index) => {
            mensagemPedido += `${index + 1}. ${pizzaJson[item.id].name} - ${item.size}\nQuantidade: ${item.qt}\n\n`;
        });
        ...
        mensagemPedido += `Subtotal: ${formatoReal(subtotal)}\n`;
        mensagemPedido += `Total: ${formatoReal(subtotal)}\n`;

        mensagemPedido = encodeURIComponent(mensagemPedido);

        const linkWhatsApp = "https://wa.me/" + numeroWhatsApp + "?text=" + mensagemPedido;
...
pizzaJson.map((item, index ) => {
    ...
    pizzaItem.querySelector('.pizza-item--img img').alt = item.name;
